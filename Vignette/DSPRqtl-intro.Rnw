...
The DSPRscan results can be plotted using the DSPRplot function. 
Multiple scan results can be plotted on the same plot.  Pass the scan 
results as a \code{list()} to the DSPRscan function.

<<dev="pdf",fig.width=6.5,fig.height=3.5>>=
